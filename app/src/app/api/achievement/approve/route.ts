...
import { getClaim, setClaim, pointsForRank } from "../store";

const ADMIN_KEY = process.env.SIGNAL_ADMIN_KEY;
const APP_URL   = process.env.NEXT_PUBLIC_APP_URL ?? "https://bangkok-node.vercel.app";
...
export async function POST(req: NextRequest) {
  ...
  if (action === "reject") {
    await setClaim({ ...claim, status: "rejected" });
  ...
  return NextResponse.json({
    success: true, claimId,
    nftMint: mintData.mint, points,
    wallet:  claim.wallet,
